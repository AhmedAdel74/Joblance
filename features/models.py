...
class Craftsmen(models.Model):
    ...
    def average_review(self):
        review = Rating.objects.filter(
            RACraftsmen=self).aggregate(average=Avg('RAting'))
        avg = 0
    ...
    def count_review(self):
        reviews = Rating.objects.filter(
            RACraftsmen=self).aggregate(count=Count('id'))
        cnt = 0
    ...
    def get_top_rated_craftsmen():
        top_craftsmen = Craftsmen.objects.annotate(
            avg_rating=Avg('rating__RAting')).order_by('-avg_rating')[:9]
...
class Rating(models.Model):
    ...
    RACraftsmen = models.ForeignKey(
        'Craftsmen', on_delete=models.CASCADE, blank=True, null=True, verbose_name=_("Craftsmen "))
