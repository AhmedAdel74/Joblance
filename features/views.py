...
def craftsman_detail(request, craftsman_id):
    ...
    context = {'craftsman_detail': craftsman_details,
               'rating_details': rating_details, 'Rev': Rev}
...
def submit_rating(request, craftsmanId):
    if request.method == 'POST':
        ...
        existing_rating = Rating.objects.filter(
            RAUser=user, RACraftsmen=craftsman).first()
        if existing_rating:
            ...
            if rating_value is None and description == '':
                ...
                messages.warning(
                    request, "Please fill in either the rating or the description field.")
            else:
                existing_rating.RAting = rating_value
                existing_rating.RADescription = description
                existing_rating.save()
                messages.success(
                    request, "The rating has been updated successfully.")
        else:
            ...
            if rating_value is None and description == '':
                ...
                rating = Rating(RAUser=user, RACraftsmen=craftsman,
                                RAting=rating_value, RADescription=description)
                rating.save()
                messages.success(
                    request, "The evaluation of Craftsmen has been completed successfully.")
